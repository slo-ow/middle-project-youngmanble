...
public class Client_board extends JFrame implements ActionListener{
	...
	private JTextField nickTxt; // nickName받는 텍스트
	...
	private void loginInit() {
		...
		loginGUI.setBounds(100, 100, 600, 490); // 로그인 창 크기
		loginPane = new JPanel();
		loginPane.setBackground(new Color(255, 182, 193));
		loginPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		loginGUI.setContentPane(loginPane);
		loginPane.setLayout(null);
		
		JLabel MainLabel = new JLabel("모두의 YongManble");
		MainLabel.setFont(new Font("굴림", Font.BOLD, 36));
		MainLabel.setBounds(120, 50, 360, 80);
		loginPane.add(MainLabel);
		
		JLabel lblNewLabel = new JLabel("Server IP");
		lblNewLabel.setFont(new Font("굴림", Font.PLAIN, 20));
		lblNewLabel.setBounds(60, 140, 78, 28);
		loginPane.add(lblNewLabel);

		JLabel lblNewLabel_1 = new JLabel("Server Port");
		lblNewLabel_1.setFont(new Font("굴림", Font.PLAIN, 20));
		lblNewLabel_1.setBounds(60, 180, 97, 25);
		loginPane.add(lblNewLabel_1);

		JLabel lblNick = new JLabel("NICK");
		lblNick.setFont(new Font("굴림", Font.PLAIN, 20));
		lblNick.setBounds(60, 220, 21, 25);
		loginPane.add(lblNick);

		ipTxt = new JTextField();
		ipTxt.setBounds(175, 140, 140, 23);
		loginPane.add(ipTxt);
		ipTxt.setColumns(10);

		portTxt = new JTextField();
		portTxt.setBounds(175, 180, 140, 23);
		loginPane.add(portTxt);
		portTxt.setColumns(10);

		nickTxt = new JTextField();
		nickTxt.setBounds(175, 220, 140, 23);
		loginPane.add(nickTxt);
		nickTxt.setColumns(10);

		loginBtn.setBounds(130, 300, 195, 30);
		loginPane.add(loginBtn);
		
		JLabel label = new JLabel("");
		label.setIcon(new ImageIcon(Client_board.class.getResource("/imgs/로그인창.jpg")));
		label.setBounds(0, 0, 584, 452);
		loginPane.add(label);
	...
	private void MainInit() {
		...
		JLabel label = new JLabel("");
		label.setIcon(new ImageIcon(Client_board.class.getResource("/imgs/Main.jpg")));
		label.setBounds(0, 0, 434, 481);
		mainPane.add(label);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == loginBtn) {
			...
			if (ipTxt.getText().length() == 0) {
			...
			} else if (nickTxt.getText().length() == 0) {
				nickTxt.setText("NickName를 입력해 주세요");
				...
				nick = nickTxt.getText().trim(); // nickName 받기
			}
